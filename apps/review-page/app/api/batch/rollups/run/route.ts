...
type ReviewRowWithFlags = ReviewRow & {
  course_review_ai_flags?: { ai_flagged: boolean }[] | null;
};
    ...
    for (const r of subjects) {
      ...
      try {
        ...
        const { data: reviews, error: revErr } = await supabaseAdmin
          .from('course_reviews')
          .select(
            'id,subject_id,created_at,credit_ease,class_difficulty,assignment_load,attendance_strictness,satisfaction,recommendation,course_review_ai_flags(ai_flagged)'
          )
        ...
        const rows = (reviews || [])
          .filter((r: ReviewRowWithFlags) => {
            const flags = r.course_review_ai_flags;
            if (!flags || flags.length === 0) return true;
            return !flags.some((f) => f.ai_flagged);
          })
          .map((r: ReviewRowWithFlags) => ({
            id: r.id,
            subject_id: r.subject_id,
            created_at: r.created_at,
            credit_ease: r.credit_ease,
            class_difficulty: r.class_difficulty,
            assignment_load: r.assignment_load,
            attendance_strictness: r.attendance_strictness,
